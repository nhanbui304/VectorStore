...
TEST_CASE("VectorStore basic operations") {
  ...
  VectorStore store(2, TestHelper::embedding);
  CHECK(store.size() == 0);
...
TEST_CASE("VectorStore helper functions") {
  TestHelper::resetNextId();
  VectorStore store(3, TestHelper::embedding);
...
TEST_CASE("VectorStore distance and similarity functions") {
  ...
  double expectedCos =
      (1 * 2 + 2 * 3) / (std::sqrt(1 * 1 + 2 * 2) * std::sqrt(2 * 2 + 3 * 3));
  ...
  CHECK(l2 == doctest::Approx(std::sqrt(
                  1 * 1 + 1 * 1)));  // sqrt((1-2)^2 + (2-3)^2) = sqrt(2)
}
...
TEST_CASE("test_208") {
  ...
  string result = "";
  for (int i = 0; i < k; i++)
    result += to_string(nearestIndices[i] + 2) + ":" +
              to_string(labels[nearestIndices[i]]) + " ";
