...
class VectorStore {
 ...
 public:
  struct VectorRecord {
    ...
    VectorRecord(int id, const string &rawText,
                 SinglyLinkedList<float> *vector);
  };
 ...
 public:
  ...
  int *topKNearest(const SinglyLinkedList<float> &query, int k,
                   const string &metric = "cosine") const;
 ...
 private:
  ...
  static bool better(const RecordScore &a, const RecordScore &b,
                     const std::string &metric);
  static void quickSort(ArrayList<RecordScore> &arr, int left, int right,
                        const std::string &metric);
};
